...
- **C2 address**: 6-layer encoding pipeline — AES-128-CTR outer layer wrapping 5 inner layers (MD5 checksum → byte substitution → RC4 → XOR rotating key → base64). The AES-encrypted blob (`encGothTits`) is decrypted at runtime by `garuda()`, then decoded by `venusaur()`
